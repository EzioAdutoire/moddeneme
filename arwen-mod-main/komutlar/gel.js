...
if (!kullanıcı) return message.channel.send('Bir Kullanıcı Belirt <a:axze_iptal:929762366692929616>');

let member = message.guild.member(kullanıcı);

if (!member.voice.channel) return message.channel.send('Etiketlenen Kullanıcı Ses Kanalında Değil <a:axze_iptal:929762366692929616>').then(m => m.delete,{timeout: 5000});
...
.setColor("0x348f36")
.setDescription(`${kullanıcı} Odaya Çekilme Teklifini Onayladı <a:axze_onay:929763543618830416>`)
message.channel.send(kabul)
kullanıcı.voice.setChannel(message.member.voice.channel.id)
} else {
let striga = new Discord.MessageEmbed()
.setColor("0x800d0d")
.setDescription(`${kullanıcı} Odaya Çekilme Teklifini Reddetti <a:axze_iptal:929762366692929616>`)
