...
return message.reply("Ses kanalında olman lazım <a:axze_iptal:929762366692929616>");
}
...
if (!member.voice.channel) return message.channel.send('Etiketlenen Kullanıcı Ses Kanalında Değil <a:axze_iptal:929762366692929616>').then(m => m.delete(5000));
...
.setColor("0x348f36")
.setDescription(`${kullanıcı} Odaya Gelmeni Onayladı <a:axze_onay:929763543618830416>`)
message.channel.send(kabul)
message.member.voice.setChannel(kullanıcı.voice.channel.id)
} else {
let striga = new Discord.MessageEmbed()
.setColor("0x800d0d")
.setDescription(`${kullanıcı} Odaya Gelmeni Onaylamadı <a:axze_iptal:929762366692929616>`)
